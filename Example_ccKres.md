...
1. Create a new folder named `Example_ccKres`.
2. Download `Example_ccKres.py` and the module `module_slovenian_ala.py` into this folder.
3. Download the XML version of the Kres corpus (`cckresV1_0.zip`) from the [CLARIN.SI repository](https://www.clarin.si/repository/xmlui/handle/11356/1034) and extract it into `Example_ccKres/cckresV1_0`.

    **Note:** A small portion of the corpus ccKres is already included in the `Example_ccKres.zip` file in this repository.
...
ala.extract_genres_from_xml(
    "Example_ccKres/cckresV1_0",
    "Example_ccKres/cckresV1_0_leposlovje",
    genre="#SSJ.T.K.L"
)
```

This creates the folder `Example_ccKres/cckresV1_0_leposlovje` and extracts only literary texts into it.  
...
ala.convert_xml_to_txt(
    "Example_ccKres/cckresV1_0_leposlovje",
    "Example_ccKres/cckresV1_0_leposlovje_txt"
)
```
...
ala.classla(
    "Example_ccKres/cckresV1_0_leposlovje_txt",
    "Example_ccKres/cckresV1_0_Classla"
)
```

The function `si_ala.classla` performes lemmatization, part-of-speech (POS) tagging and annotation of universal dependencies (UD) relations. The output will be saved as `.xlsx` files in `Example_ccKres/cckresV1_0_Classla`. The xlsx files will contain columns "word", "lemma", "pos" and "deprel".
...
**Note:** Results of this analysis are in the `Example_ccKres.zip` file in this repository.